/*
    There is no compilation for JS, its only interpreted    
    Single quote and double quotes can be used interchangably
    JS creates objects for every primitives so we are able to access string.length
    Function overloading is not available in JS, if function is declared with 2 param and while calling we call with 1 param then no error 
        here 2nd param is undefined, if we call with 3 param then also no error
    === is used for comparision with data type values
...
//1. dot(.) notation, it is faster for performance as interpreter knows key in advance. If property is number can not use . notation
//2. [] nothation 
/*
let person = {
    name : 'Vaibhav',
...
//Use of . over [] while retrieving
/*
let person = {
    name : "Vaibhav",
    age : 30
};
var dynamicIndex = "name";
console.log(person['name']);
console.log(person[dynamicIndex]);
*/
...
console.log(person.children.childname);
//Setting new value to inner object
person.children.address = "West 21";
console.log(person.children.address);
...
console.log(selectedColor[0]);  //Array is object in JS but we can not use . notation as keys are number here
console.log(selectedColor["0"]);  //This is same as that of above line, JS internally convert above line to this one
...
//METHOD 2 for ITERATION using function -- These are array functions which accepts function as an arguments
/*
